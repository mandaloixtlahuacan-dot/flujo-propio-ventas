...
/** Only clear BUY / call intent from the *customer* — not bot-mentioned words. */
const HOT_KEYWORDS = [
  "quiero transferir",
  "pasame la clabe",
  "pásame la clabe",
  "pasame clabe",
  "quiero la videollamada",
  "quiero videollamada",
  "agendemos",
  "agendar llamada",
  "voy a pagar",
...
export function detectHotSignals(text: string): {
  ...
  if (
    lower.includes("quiero videollamada") ||
    lower.includes("quiero la videollamada") ||
    lower.includes("agendemos") ||
    lower.includes("agendar llamada") ||
    ...
    lower.includes("ya transfer") ||
    lower.includes("deposite") ||
    lower.includes("deposité") ||
    lower.includes("voy a pagar") ||
...
export async function upsertLead(params: {
  ...
  if (params.appendHistory?.length) {
    ...
    // if both in same batch, mark each
